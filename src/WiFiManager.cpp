...
void WiFiManager::setContentText(String& title, String& name, String& maker, String& version)
{
    _title = title;
    _name = name;
    _maker = maker;
...
void WiFiManager::cycleStateMachine()
{
    ...
    switch(_currentState) {
        ...
        case SAVE_CREDENTIALS:
            ...
            LOG_DEBUG("Rebooting to connect with new credentials");
            WiFi.disconnect(true);
            
            delay(1000);  // Delay for any pending operations
            NVIC_SystemReset();  // Reset the Pico
...
bool WiFiManager::startConfigPortal()
{
    ...
    if (!_setupConfigPortal)
    {
        ...
        pixels.clear();
        pixels.setPixelColor(0, pixels.Color(0, 32, 0, 0)); // Green
        pixels.setPixelColor(1, pixels.Color(32, 32, 0, 0)); // Green
        pixels.setPixelColor(2, pixels.Color(0, 0, 32, 0)); // Green
        pixels.show();
...
bool WiFiManager::loadCredentials()
{
    ...
    LOGF_DEBUG("Retrieved credentials for SSID: %s pass: %s and CRC %d", temp._ssid, temp._pwd, temp._crc);
...
void WiFiManager::saveCredentials()
{
    ...
    LOGF_DEBUG("Stored credentials for SSID: %s pass: %s and CRC: %d", temp._ssid, temp._pwd, temp._crc);
}
